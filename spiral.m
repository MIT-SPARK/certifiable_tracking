...
%  Rs = repmat(eye(3)*rand_rotation('RotationBound',0.1), 1,1,L);
% dRs = repmat(axang2rotm([0,0,1,1])*rand_rotation('RotationBound',0.1),1,1,L-1);
% for l = 2:L
%     Rs(:,:,l) = Rs(:,:,l-1)*dRs(:,:,l-1);
% end
...
%     pts = get_spiral_pts(R, dR, v, p, dt/(num_intermediate), num_intermediate + 1);
    pts = sim_dynamics(R, dR, v, p, dt, num_intermediate);
