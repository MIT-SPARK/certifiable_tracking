function solution = gnc_test(problems,SDP,path,varargin)
...
while (true)
    ...
    weights2 = reshape(weights,length(problems),problem.N);

    residuals = zeros(length(problems),problem.N);
    for l = 1:length(problems)
    problems{l}.weights = weights2(l,:);
    [R,t,c,ofinfo]      = outlier_free_category_registration(problems{l},'lambda',lambda);
    shape               = combine_shapes(shapes,c);
    residuals(l,:)      = sum( (scene - R*shape - t).^2, 1) / noiseBoundSq;
    % f_cost              = sum(min(residuals,barc2)) + lambda * (c'*c);
    end
    residuals = reshape(residuals,[length(problems)*problem.N,1]);
    f_cost = residuals*weights;% + lambda*(c'*c);
