...
- **Rotation matrix sampling.** For synthetic data (including the PASCAL data), we generated the ground truth rotations using a method which is biased towards the poles (not uniform as reported in the paper). This erroneous sampling is inherited from the [CertifiablyRobustPerception](https://github.com/MIT-SPARK/CertifiablyRobustPerception/blob/master/utils/rand_rotation.m) project. We reran synthetic experiments and there was no clear difference in the resutls. Real-world experiments (NOCS, YCBInEOAT, and drone) are unaffected.

![](media/fixed_rotations.png)
Top row: the outlier experiments reported in the paper, where synthetic data was generated from incorrectly sampled rotations. Bottom row: the outlier experiments after correcting the rotation sampling. There is no clear difference between the two except for the position uncertainty which is an artifact of GNC.
