...
- **Rotation matrix sampling.** For synthetic data (including the PASCAL data), we generated the ground truth rotations using a method which is biased towards the poles (not uniform as reported in the paper). This erroneous sampling is inherited from the [CertifiablyRobustPerception](https://github.com/MIT-SPARK/CertifiablyRobustPerception/blob/master/utils/rand_rotation.m) project. We reran synthetic experiments and there was no clear difference in the results. Real-world experiments (NOCS, YCBInEOAT, and drone) are unaffected.
