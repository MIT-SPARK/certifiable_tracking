...
class CONSTBODY:
    ...
    def f(cls, state, omega, noise, dt):
        ...
        dR_new = state.dR.dot(SO3.exp(noise[3:6]))

        # rotation
        R_new = (state.R).dot(state.dR)
        # translation
        p_new = state.p + state.R @ state.v*dt
    ...
    def h(cls, state):
        ...
        """
        y = np.hstack([np.squeeze(state.p), SO3.log(state.R)])
        return y
    ...
    def phi_inv(cls, state, hat_state):
        ...
        xi = np.hstack([
                ...
                SO3.log(hat_state.R.T @ (state.R)),
                np.squeeze(state.v - hat_state.v),
                SO3.log(hat_state.dR.T @ (state.dR)),
                ])
        return xi
    

def run_ukf(dt, L, p_meas, R_meas, v_init, dR_init, Q, R, P):
    '''
    ...
    # initial state
    state0 = model.STATE(R=R_meas[:,:,0], p=np.squeeze(p_meas[:,:,0]), v=v_init, dR=dR_init)
    ...
    p_est = np.zeros([3,1,L-1])
    R_est = np.zeros([3,3,L-1])
